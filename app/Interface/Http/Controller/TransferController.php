...
use App\Application\UseCase\TransferFunds;
...
final class TransferController
{
  public function create(): void
  {
    try {
      ...
      $fromHex = (string)($data['from_wallet_id'] ?? '');
      ...
      if ($idemKey === '') {
        // fallback: body key (allowed for testing)
        $idemKey = (string)($data['idempotency_key'] ?? '');
      }

      $uc = new TransferFunds(
        $db,
      ...
      // basic status mapping (we’ll refine later)
      $msg = $e->getMessage();
      $code = str_contains($msg, 'Insufficient') ? 409 : 400;
      JsonResponse::send($code, ['error' => $msg]);
    }
  }
}
